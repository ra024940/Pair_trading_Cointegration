...
Time series forecasting can be re-framed as a supervised learning problem, and thats what we did, instead of treating the Z-Score as a signal and try to predict its behavior in the future <a href="#1g">**(3)**</a>, we framed the problem as a binary classification, training the model to predict if a trade would be profitable or not, based on the variables (features) at the very moment the trade is to be opened. To do that, we had to get rid of all information that could expose our model to future-looking, since some features have a temporal dependency between observations, the model could only decorate these relationships and overfit the training set. To compensate that we incorporated some seasonal variables that reflected market condition to be features (Ex. Volatility, Number of trades generated by the pair in the last N days).
